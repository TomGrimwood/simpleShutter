...
static void shutter_sensor_update_mode_config_internal() {
    ...
    ESP_LOGI(TAG, "Updating mode config for mode: %d", current_mode);
    switch (current_mode) {
    ...
    ESP_LOGI(TAG, "Mode config updated. S1_active:%d, S2_active:%d, S3_active:%d", is_sensor1_active, is_sensor2_active, is_sensor3_active);
}
...
void shutter_sensor_get_raw_times(uint64_t* s1_o, uint64_t* s1_c, uint64_t* s2_o, uint64_t* s2_c, uint64_t* s3_o, uint64_t* s3_c) {
    portENTER_CRITICAL(&timer_spinlock);
    if(s1_o) *s1_o = sensor1_open_time_us; if(s1_c) *s1_c = sensor1_close_time_us;
    if(s2_o) *s2_o = sensor2_open_time_us; if(s2_c) *s2_c = sensor2_close_time_us;
    if(s3_o) *s3_o = sensor3_open_time_us; if(s3_c) *s3_c = sensor3_close_time_us;
    portEXIT_CRITICAL(&timer_spinlock);
}
...
bool shutter_sensor_is_active(uint8_t sensor_gpio, measurement_mode_t mode_to_check) {
    ...
    // is_sensorX_active based on current_mode.
...
void shutter_sensor_init(void) {
    ...
    portENTER_CRITICAL(&timer_spinlock);
    // current_mode is already MODE_ALL_SENSORS by default static initialization
    shutter_sensor_update_mode_config_internal(); 
    portEXIT_CRITICAL(&timer_spinlock);
